...
def PrediksiProdi(biologi, fisika, inggris):
...
# Membuat atau menyimpan data ke database SQLite
def simpan_data_ke_sqlite(nama_siswa, biologi, fisika, inggris, prediksi_fakultas):
    conn = sqlite3.connect("PrediksiPD.db")
    cursor = conn.cursor()

# Membuat tabel jika belum ada
    cursor.execute('''CREATE TABLE IF NOT EXISTS Prediksi_Prodi
...
# Memasukkan data mata pelajaran ke dalam tabel
    cursor.execute('''INSERT INTO Prediksi_Prodi
                (nama_siswa,
                biologi, 
                fisika, 
                inggris, 
...
# Label Nama Siswa
label_namaSis = tk.Label(frame_input, text="Nama Siswa : ")
label_namaSis.grid(row=0, column=0, pady=10)
entry_namaSis = tk.Entry(frame_input)
entry_namaSis.grid(row=0, column=1)
...
# Label Nilai inggris
label_hasil3 = tk.Label(frame_input, text="Nilai Ingris : ")
